...
Suppose I ask you to plot each subject's reaction time (y-axis) vs the number of days of sleeacep restriction (x-axis). "Sketch" out in words what the first few rows of the data need to look like in order to do this. It might help to think about what you'd need to complete the plotting frame:

`ggplot(sleep, aes(y = reaction_time, x = day, color = subject))`
...
sleep_wide |>
  ...
  head(3)
# day is chr, need to be int; prefix is not removed.
```
...
Created a new dataset called sleep_long
...
```{r}
ggplot(sleep_long, aes(x=day, y=reaction_time, color = Subject))+geom_line()
head(sleep_long)
#This is because Subjects are integers but days are strings
```
...
ggplot(sleep_long, aes(x=day, y=reaction_time, color = Subject)) + geom_line() + facet_wrap(~Subject)
```
...
Generally, the more days you're sleep deprived, the longer the reaction time.
-   Is this the same for everybody? What differs?
However, some poeple are not affected and always react fast. Some people get gradually slowlier. Some people gets slow first and then get very fast for some reason.
...
```{r}
billboard |>
  ggplot(aes(x=wk1, y=wk2)) + geom_point()+geom_abline(intercept = 0, slope = 1)
```
...
Create and discuss a visualization of the rankings (y-axis) over time (x-axis) for the 2 songs in `nov_1999`. There are hints below (if you scroll), but you're to play around and use as few hints as possible.

```{r}
nov_1999 |>
  pivot_longer(cols = -artist,  values_to = "ranking", names_to = "week", names_prefix = "wk")|>
  mutate(week = as.numeric(week))|>
  ggplot(aes(x=week, y=ranking, color=artist)) + geom_line()
```
