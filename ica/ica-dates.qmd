...
We're deleting too many data points with na.omit(). 
...
penguins |>
  select(c(species, contains("length")))
```
...
# First just says 2nd month while second writes out Feb
month(today)
...
# Which week today is in this year
week(today)
```

```{r}
# What do these lines produce / what's their difference?
# Which day today is in the month / year.
mday(today)
...
```{r}
# What do these lines produce / what's their difference?
# First just says it is 4th day in the week while the second writes out Wed.
wday(today)
...
# Today is later than 2024-02-14 and not ea
